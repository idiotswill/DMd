...
<div class="table-app" aria-busy={busy}>
  ...
  {#if view}
    ...
    {:else}
      ...
      {#if view.tactical}{#key view.tactical.encounter_id}<EncounterPanel tactical={view.tactical} characters={view.characters} {host} actor={currentCharacter?.entity_id??null} player={playerId||null} disabled={locked||!!view.pending||!view.active_session} pendingRoll={!!view.roll} onAction={(action)=>act({Tactical:{action}})}/>{/key}{/if}
