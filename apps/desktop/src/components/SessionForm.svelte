<script lang="ts">
  import type { CharacterView, Participant, Player } from '../table-api';
  let { players, characters, disabled = false, onStart }: { players: Player[]; characters: CharacterView[]; disabled?: boolean; onStart: (name: string, participants: Participant[]) => void } = $props();
  let name = $state('');
  ...
  function start(event: SubmitEvent) {
    ...
    if (!participants.some(p => p.attendance === 'Present' && p.character_id)) { error = 'Select at least one attending player and their character.'; return; }
