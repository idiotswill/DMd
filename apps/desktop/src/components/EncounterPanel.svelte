...
<section class="panel"><h2>Encounter{tactical.round ? ` · round ${tactical.round}` : ''}</h2>
  ...
  {#if tactical.hit}
    {#key `${host}:${player}:${actor}:${tactical.hit.order?.key}:${tactical.hit.delegate}:${tactical.hit.response?.key}`}
      <HitResponsePanel hit={tactical.hit} {actor} {host} disabled={disabled||pendingRoll} {onAction}/>
    {/key}
  {/if}
  ...
  {#if !legacy && tactical.phase==='active' && tactical.budget && (host || actor===tactical.active_actor)}
  ...
  {/if}
  {#if !legacy && tactical.area_options && (host || actor===tactical.area_options.actor)}
  ...
  {/if}
  {#if !legacy && tactical.casting_options && (host || actor===tactical.casting_options.actor)}
  ...
  {/if}
  {#if !legacy && tactical.shield_options && (host || actor===tactical.shield_options.actor)}
  ...
  {/if}
  {#if !legacy && tactical.attack_options && (host || actor===tactical.attack_options.actor) && tactical.attack_options.weapons.some(weapon=>weapon.purposes.length>0)}
  ...
  {/if}
  {#if !legacy && tactical.movement_options && (host || actor===tactical.movement_options.actor)}
