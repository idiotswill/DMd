...
<section class="panel"><h2>Encounter{tactical.round ? ` · round ${tactical.round}` : ''}</h2>
  ...
  {#if tactical.legendary_action && (host || actor===tactical.legendary_action)}
    <fieldset {disabled}><legend>Legendary Action opportunity</legend><p>{name(tactical.legendary_action)} has an after-turn opportunity. Its feature actions are not available in this build.</p><button class="secondary" onclick={()=>onAction('DeclineLegendaryAction')}>Pass this opportunity</button></fieldset>
  {/if}
</section>
