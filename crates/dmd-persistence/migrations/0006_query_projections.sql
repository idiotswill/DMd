...
CREATE TABLE projection_scenes (
    ...
    location_id TEXT NOT NULL,
    mode_json TEXT NOT NULL CHECK (json_valid(mode_json)),
    status TEXT NOT NULL,
...
BEGIN
    INSERT INTO projection_heads VALUES (
        ...
        json_array_length(json_extract(NEW.state_json, '$.players')),
        json_array_length(json_extract(NEW.state_json, '$.characters')),
        json_array_length(json_extract(NEW.state_json, '$.entities')),
        json_array_length(json_extract(NEW.state_json, '$.factions')),
        json_array_length(json_extract(NEW.state_json, '$.locations')),
        json_array_length(json_extract(NEW.state_json, '$.scenes')),
        (SELECT COUNT(*) FROM json_each(NEW.state_json, '$.scenes') s, json_each(s.value, '$.presences')),
        json_array_length(json_extract(NEW.state_json, '$.items')),
        json_array_length(json_extract(NEW.state_json, '$.facts')),
        json_array_length(json_extract(NEW.state_json, '$.claims')),
        json_array_length(json_extract(NEW.state_json, '$.beliefs')),
        json_array_length(json_extract(NEW.state_json, '$.knowledge')),
        json_array_length(json_extract(NEW.state_json, '$.directives'))
    );
